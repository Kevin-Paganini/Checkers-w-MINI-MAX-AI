...
from checkers.constants import BLACK, ROWS, COLS, SQUARE_SIZE, RED, WHITE
import pygame
...
class Board:
    ...
    def _traverse_left(self, start, stop, step, color, left, skipped=[]):
        ...
        for r in range(start, stop, step):
            if left < 0: #going off the board
                break
            current = self.board[r][left]
            if current == 0: #the next spot is empty
            ...
            elif current.color == color: # the next spot is the same color as the piece in question
                break    
            else:   
                last = [current]        
    ...
    def evaluate(self):
        ...
        result = self.white_left - self.red_left + (self.white_kings * 0.5 - self.red_kings * 0.5)
        return result
